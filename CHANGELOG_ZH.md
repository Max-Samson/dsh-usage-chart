...
- 在 DSH 0.1.2+ 从独立的 `chat` 源读取会话节点，旧版继续回退到会话快照；恢复 PR #12/#15 合并时被覆盖的指示器接线，并补充组件级回归测试。
- 按最近的 fixed 定位包含块放置展开面板，适配为 dock 添加 `backdrop-filter` 的皮肤（PR #14）。
- 会话成本按各轮实际模型和计费时段求和（PR #15）。实时 token 超过已读取历史时，为新增 token 加上带估算标记的费用，短暂静默后刷新宿主历史；指示器和面板共用同一份成本结果。
- 统一指示器与面板的模型归因顺序：优先使用已追上实时用量的宿主折叠结果；旧版节点回退只订阅节点列表，避免无关快照更新触发重渲染。
